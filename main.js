...
const validateCred = (array) => {
  ...
  for (i=array.length - 1; i > -1; i--) {
    //Double every other number and subtract 9 if that sum is greater than 9
    let currNum = array[i]
    // console.log(currNum)
    if (i % 2 == 0) {
      currNum = currNum * 2
      // console.log(`Number Multiplied: ${currNum}`)
    }
    if (currNum > 9) {
      currNum = currNum-9
      // console.log(`Number Subtracted: ${currNum}`)
    }
    testArray.push(currNum)
  }
  //sum all elements in the test array into a new number
  // console.log(testArray)
  let counterSum = 0;
  const sumForTesting = testArray.reduce((previousValue, currentValue) => previousValue + currentValue, counterSum)
  if (sumForTesting % 10 == 0) {
    return true
  } else {
    return false
  }
